"""
Croissant MCP Server with Cursor-compatible API key authentication and SSE handling
"""
import os
import json
    ...
    if request.headers.get("Accept") == "text/event-stream":
        try:
            ...
            if body_bytes:
                ...
                if isinstance(body, dict) and "env" in body and "API_KEY" in body["env"]:
                    if body["env"]["API_KEY"] == API_KEY:
                        logger.info(f"Valid API key provided in request body for {request.url.path}")
                        return True
...
async def info():
    return {
        ...
        "endpoints": [
            ...
            {"path": "/mcp", "method": "GET", "description": "MCP SSE endpoint"},
...
async def mcp_sse(request: Request):
    ...
    logger.info(f"SSE connection from client to {request.url.path}")
